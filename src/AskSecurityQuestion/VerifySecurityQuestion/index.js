...
exports.VerifySecurityQuestion = async (req, res) => {
	...
	if (req.method === "OPTIONS") {
		...
		switch (req.method) {
			case "POST":
				await verifySecurityQuestion(req, res);
				break;
			default:
...
const verifySecurityQuestion = async (req, res) => {
	try {
		let response = await verifySecurityQuestionData(req);
...
const verifySecurityQuestionData = async (req) => {
	let headerPassword = process.env.headerPassword;
	return await axios.post(
		`${process.env.baseUrl}/verify-security-questions`,
		req.body,
		{
			headers: {
